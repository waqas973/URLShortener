...
<script>
    ...
    <?php 
      ...
      $sql = "select browser_name,clicks , sum(clicks) as visit from detection where customer_id=:customer_id group by browser_name ";
  ...
  $sql = "select device_type,clicks , sum(clicks) as visit from detection where customer_id=:customer_id group by device_type ";
  $stmt = $conn->prepare($sql);
  $params = array(
    "customer_id"=>$customer_id
) ;
...
?>
  ...
  $sql = "select OS_type,clicks , sum(clicks) as visit from detection where customer_id=:customer_id group by OS_type ";
  $stmt = $conn->prepare($sql);
  $params = array(
    "customer_id"=>$customer_id
) ;
...
?>
  ...
  $sql = "select country,sum(clicks) as visit from detection where customer_id=:customer_id group by country ";
  $stmt = $conn->prepare($sql);
  $params = array(
    "customer_id"=>$customer_id
) ;
